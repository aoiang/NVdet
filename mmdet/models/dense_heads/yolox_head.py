...
class YOLOXHead(BaseDenseHead, BBoxTestMixin):
    ...
    def forward(self, x1, x2, x3):
        ...
        print("===========here is the size of head input===================")
        # for d in feats:
        #     print(d.size())
...
@HEADS.register_module()
class NASYOLOXHead(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
        ...
        from .network_json import network
        self.arch = network['head']
        self.layers = nn.ModuleDict()
        for i in range(len(self.arch['id'])):
            if self.arch['id'][i][1]['type'] == 'ConvModule':
                padding = 1 if self.arch['id'][i][1]['kernel'] == 3 else 0
                self.layers[str(i)] = ConvModule(int(self.arch['id'][i][1]['in_channel']),
            ...
            elif self.arch['id'][i][1]['type'] == 'Conv':
                padding = 1 if self.arch['id'][i][1]['kernel'] == 3 else 0
                self.layers[str(i)] = nn.Conv2d(int(self.arch['id'][i][1]['in_channel']),
                                                int(self.arch['id'][i][1]['out_channel']),
                                                self.arch['id'][i][1]['kernel'])
    ...
    def init_weights(self):
        super(NASYOLOXHead, self).init_weights()
        # Use prior in model initialization to improve stability
        bias_init = bias_init_with_prob(0.01)
        for conv_cls, conv_obj in zip(self.multi_level_conv_cls,
                                      self.multi_level_conv_obj):
    ...
    def forward_single(self, x, layers):
        """Forward feature of a single scale level."""
